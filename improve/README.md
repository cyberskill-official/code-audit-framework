# improve/ — the self-improvement loop, mapped on one screen
...
| [`retros/`](./retros/) | **Scored history.** One filled rubric per run or critic cycle. `evals/scripts/retro-summary.py` aggregates these into per-version trends. | Add a file after every run. Append-only. |
...
`AUDIT.md`, gated by `evals/`, snapshotted to `versions/`, logged in
